...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 my-5 mx-2 sm:mx-5">
        @foreach($posts as $post)
            <a href="{{ route('posts.show', ['post' => $post->id, 'slug' => $post->slug]) }}" class="relative flex md:flex-row w-full gap-2 bg-slate-50 dark:bg-slate-800 p-4 shadow-md dark:shadow-gray-950 rounded-xl">
                @if ($image)
